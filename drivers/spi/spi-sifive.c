...
static void sifive_spi_prep_device(struct sifive_spi *spi, struct spi_device *device)
{
	...
	/* Switch format mode bits */
	cr = sifive_spi_read(spi, XSPI_FFR_OFFSET) & ~XSPI_FF_LSB_FIRST;
	if (device->mode & SPI_LSB_FIRST)
	  cr |= XSPI_FF_LSB_FIRST;
	sifive_spi_write(spi, XSPI_FFR_OFFSET, cr);
}

static void sifive_spi_prep_transfer(struct sifive_spi *spi, struct spi_device *device, struct spi_transfer *t)
{
	u32 hz, scale;

	/* Calculate and program the clock rate */
	hz = t->speed_hz ? t->speed_hz : device->max_speed_hz;
	scale = (DIV_ROUND_UP(clk_get_rate(spi->clk), 2 * hz) - 1) & XSPI_SCD_SCALE_MASK;
...
{
	if (!tx_ptr) {
		sifive_spi_write(spi, XSPI_TXDR_OFFSET, 0);
		return;
	}
...
static void sifive_spi_execute(struct sifive_spi *spi, struct spi_transfer *t)
{
	...
	while (remaining_words) {
		...
		sifive_spi_write(spi, XSPI_IER_OFFSET, XSPI_RXWM_INTR);
...
static int sifive_spi_transfer_one_message(struct spi_master *master, struct spi_message *msg)
{
	...
	int is_last, cs_change, cs_enable;
	...
	list_for_each_entry(t, &msg->transfers, transfer_list) {
		...
		sifive_spi_prep_transfer(spi, device, t);
		sifive_spi_execute(spi, t);
